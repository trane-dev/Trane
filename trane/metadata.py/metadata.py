...
class Metadata:
    ...
    def set_time_index(self, table, column):
        self.time_indexes[table] = column
        self.ml_types[table][column] = Datetime(tags={"time_index"})

    def set_index(self, table, column):
        self.indexes[table] = column
        index_ml_type = self.get_ml_type(table, column)
        self.ml_types[table][column] = index_ml_type.tags.add("primary_key")
