import pandas as pd

from trane.typing.column_schema import ColumnSchema
from trane.typing.inference_functions import (
    boolean_func,
    categorical_func,
    datetime_func,
    double_func,
    integer_func,
    natural_language_func,
)
from trane.typing.logical_types import (
    Boolean,
    Categorical,
    Datetime,
    Double,
    Integer,
    NaturalLanguage,
    Unknown,
)


def _infer_series_type(series):
    inference_functions = {
        boolean_func: ColumnSchema(logical_type=Boolean),
        categorical_func: ColumnSchema(
            logical_type=Categorical,
            semantic_tags={"category"},
        ),
        datetime_func: ColumnSchema(logical_type=Datetime),
        double_func: ColumnSchema(logical_type=Double, semantic_tags={"numeric"}),
        integer_func: ColumnSchema(logical_type=Integer, semantic_tags={"numeric"}),
        natural_language_func: ColumnSchema(logical_type=NaturalLanguage),
    }
    for infer_func, logical_type in inference_functions.items():
        if infer_func(series) is True:
            return logical_type
    return Unknown


def infer_table_meta(df):
    table_meta = {}
    if isinstance(df, pd.Series):
        column_schema = _infer_series_type(df)
        table_meta[df.name] = column_schema
        return table_meta
    elif isinstance(df, pd.DataFrame):
        for col in df.columns:
            column_schema = _infer_series_type(df[col])
            table_meta[col] = column_schema
    else:
        raise ValueError("df must be either a pandas DataFrame or Series")
    return df
